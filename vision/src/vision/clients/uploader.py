    ...
    def upload_pdf_statement(
        ...
        try:
            ...
            if bank:
                data["bank"] = bank
            resp = self.session.post(
                f"{self.base_url}/api/v1/pdf/upload",
            ...
            if exc.response is not None:
                with suppress(ValueError):
                    detail = exc.response.json().get("detail")
            logger.warning("uploader POST /api/v1/pdf/upload failed: %s", detail or exc)
            return {"error": detail or str(exc)}
        except requests.RequestException as exc:
            logger.warning("uploader POST /api/v1/pdf/upload failed: %s", exc)
            return None

    def list_pdf_statements(self) -> list[dict] | None:
        try:
            resp = self.session.get(
                f"{self.base_url}/api/v1/pdf/files",
                timeout=self.timeout,
            )
            resp.raise_for_status()
            return resp.json()
        except requests.RequestException as exc:
            logger.warning("uploader GET /api/v1/pdf/files failed: %s", exc)
            return None

    def delete_pdf_statement(self, bank: str, filename: str) -> bool:
        try:
            resp = self.session.delete(
                f"{self.base_url}/api/v1/pdf/files/{bank}/{filename}",
                timeout=self.timeout,
            )
            resp.raise_for_status()
            return True
            ...
            logger.warning(
                "uploader DELETE /api/v1/pdf/files/%s/%s failed: %s", bank, filename, exc
            )
            return False
