...
def _format_error_detail(detail: object) -> str | None:
    """Flatten FastAPI's default pydantic validation-error body into one readable
    string, instead of leaking the raw `[{"loc": ..., "msg": ..., "type": ...}]`
    list to the user (which also can't be `urllib.parse.quote()`-encoded as-is
    for the error redirect, since it isn't a string)."""
    if isinstance(detail, list):
        messages = [item["msg"] for item in detail if isinstance(item, dict) and item.get("msg")]
        return "; ".join(messages) if messages else None
    return detail


def _translate_tax_estimate_error(detail: str) -> str:
    """Map the "Becsult bevetel" save path's backend validation detail to a
    plain Hungarian sentence, instead of leaking pydantic's English wording
    into an otherwise fully Hungarian UI.

    A negative `gross_revenue` (the backend's `Field(ge=0)`) is realistically
    the only validation error a user can trigger from this form -- month and
    year are generated by the template, not typed -- so a targeted message
    for that case plus a generic Hungarian fallback for anything else is
    enough; this deliberately isn't a general-purpose pydantic-message
    translator."""
    if "greater than or equal to 0" in detail:
        return "A becsült bevétel nem lehet negatív szám."
    return "Érvénytelen érték a becsült bevétel mezőben."
...
class InvoiceCoreClient:
    ...
    def _write_json(
        ...
        error_message: str,
        label: str | None = None,
        translate_detail: Callable[[str], str] | None = None,
    ) -> dict:
        ...
        `translate_detail`, when given, only runs on an actual backend-supplied
        detail (never on the generic `error_message`/network-down fallbacks,
        which are already Hungarian) -- see `_translate_tax_estimate_error` for
        ...
        try:
            ...
            if detail and translate_detail:
    ...
    def get_tax_estimate_overrides(self, year: int | None = None) -> dict:
        params = {"year": year} if year is not None else None
        return self._get_one("/api/v1/reports/tax-estimate/overrides", params) or {}

    def save_tax_estimate_overrides(self, year: int, months: list[dict]) -> dict:
        return self._write_json(
            "PUT",
            "/api/v1/reports/tax-estimate/overrides",
            {"year": year, "months": months},
            "Nem sikerült menteni a becsült bevételeket",
            label="Becsült bevétel mentése",
