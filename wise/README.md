...
uv run uvicorn wise_invoice.api.main:app --host 0.0.0.0 --port 8003 --reload

# CLI (telepítve: `wise-invoice` script)
uv run wise-invoice status                                        # Wise API kapcsolat + profilok ellenőrzése
uv run wise-invoice balances                                      # elérhető pénznemek és balance ID-k
uv run wise-invoice sync                                          # utolsó 30 nap, WISE_ACCOUNT_CURRENCY
uv run wise-invoice sync --start 2026-05-01 --end 2026-05-31
uv run wise-invoice sync --start 2026-05-01 --currency HUF
uv run wise-invoice sync --json                                   # géppel olvasható JSON kimenet
uv run wise-invoice --verbose sync --start 2026-05-01            # DEBUG szintű napló
uv run wise-invoice statements                                    # letöltött kivonat CSV-k listázása
uv run wise-invoice statements --currency HUF --from 2026-05-01
uv run wise-invoice balance-statements                            # legfrissebb kivonat tranzakciói (mint GET /balance-statements)
uv run wise-invoice balance-statements --currency HUF --from 2026-05-01  # fájllista szűréssel
uv run wise-invoice balance-statements --json                     # JSON kimenet
uv run wise-invoice import statement_25546267_HUF_2026-05-19_2026-06-02.csv
uv run wise-invoice import statement_25546267_HUF_2026-05-19_2026-06-02.csv --json
...
2. `GET /v1/profiles/{profileId}/balance-statements/{balanceId}/statement.json?intervalStart=...&intervalEnd=...` — lekéri a kivonatot
  ...
  "transactions": [
    {
      ...
      "type": "CREDIT",
      "transaction_date": "2026-05-15 10:30:00",
      "date": "2026-05-15",
      "amount": "1500.00",
      "currency": "EUR",
      "description": "Átutalás: INV-2026-42",
      "counterparty_name": "ACME Corp",
      "payee_account_number": "GB29NWBK60161331926819",
...
**Válasz mezők (`TransactionSummary`):**

| Mező                   | Leírás                                              |
|------------------------|-----------------------------------------------------|
| `wise_transaction_id`  | Wise referenciaszám (idempotencia kulcs)            |
| `type`                 | `CREDIT` vagy `DEBIT`                               |
| `transaction_date`     | Tranzakció dátuma és ideje (`yyyy-mm-dd hh:mm:ss`)  |
| `date`                 | Tranzakció napja (`yyyy-mm-dd`, computed field)     |
| `amount`               | Összeg (pozitív mindkét típusnál)                   |
| `currency`             | Pénznem (pl. `EUR`)                                 |
| `counterparty_name`    | Partner neve (küldő / fogadó / merchant)            |
| `payee_account_number` | Partner bankszámlaszáma (ha elérhető)               |
| `payment_reference`    | Fizetési referencia / közlemény                     |

### GET /transactions/{wise_transaction_id}

Egy korábban lekért tranzakció részletei azonosító alapján (in-memory keresés, max. 100 szinkron session).
...
```bash
uv run wise-invoice --verbose sync --start 2026-05-01
```
...
```bash
uv run wise-invoice sync [OPTIONS]
```
...
```bash
uv run wise-invoice statements [OPTIONS]
```
...
```bash
uv run wise-invoice balance-statements [OPTIONS]
```
...
uv run wise-invoice balance-statements

# Fájllista szűréssel
uv run wise-invoice balance-statements --currency HUF --from 2026-05-01

# JSON kimenet
uv run wise-invoice balance-statements --json
```
...
```bash
uv run wise-invoice import <FILENAME> [--json]
```
...
```
2026-06-14 10:00:00 INFO     wise_invoice.sync: Wise lekérés: 2026-05-01..2026-05-31 (EUR)
2026-06-14 10:00:01 INFO     wise_invoice.client: Wise statement 2026-05-01..2026-05-31 (EUR, balance_id=11111111): 2 tranzakció 843ms alatt
2026-06-14 10:00:01 INFO     wise_invoice.sync: Wise lekérés kész: 2 tranzakció
```
