"""
RAG Page - Document search endpoint.
"""
...
SEARCH_ENDPOINT = f"{BASE_URL}/search-documents"
...
def main():
    ...
    # Header
    st.title("🔍 RAG Search")
    st.markdown("Search documents using /search-documents endpoint")
    
    # Show endpoint configuration
    with st.expander("⚙️ Configuration", expanded=False):
        st.code(f"Endpoint: {SEARCH_ENDPOINT}\nTimeout: {REQUEST_TIMEOUT}s", language="text")
        
        # Connection test button
        col1, col2 = st.columns([1, 3])
        with col1:
            if st.button("🔌 Test Connection", key="test_rag"):
    ...
    with col2:
        if st.button("🗑️ Clear", use_container_width=True):
            logger.info(f"[CLEAR] Cleared {len(st.session_state.rag_queries)} queries")
            st.session_state.rag_queries = []
            st.session_state.rag_responses = []
            st.rerun()
    ...
    if send_button and user_input.strip():
        # Add to history
        st.session_state.rag_queries.append(user_input)
        logger.info(f"[RAG_REQUEST] Calling {SEARCH_ENDPOINT} with query: {user_input[:100]}")
        
        # Show loading state
        with st.spinner(f"Calling {SEARCH_ENDPOINT}..."):
            try:
                payload = {"message": user_input}
                logger.info(f"[RAG_REQUEST] Payload: {payload}")
                ...
                logger.info(f"[RAG_SUCCESS] status={response.status_code}")
                
                # Parse response
                try:
                    json_response = response.json()
                    st.session_state.rag_responses.append(json_response)
                    logger.info(f"[RAG_RESPONSE] {json.dumps(json_response)[:100]}")
                except json.JSONDecodeError:
                    # Response is not JSON, store as-is
                    st.session_state.rag_responses.append(response.text)
                    logger.warning(f"[RAG_NON_JSON] {response.text[:100]}")
                
                st.rerun()
                
            except requests.exceptions.ConnectionError as e:
                error_msg = f"❌ Cannot connect to {SEARCH_ENDPOINT}"
                logger.error(f"[ERROR] ConnectionError: {error_msg}")
                st.session_state.rag_responses.append(None)
                st.error(error_msg)
