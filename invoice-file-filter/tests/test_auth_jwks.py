"""JWKS TLS trust store + hibaüzenet tesztek a invoice_file_filter.auth modulhoz.
...
import invoice_file_filter.auth as auth_mod
...
def _make_request(method: str, path: str = "/api/v1/invoices/extract", token: str = "valid-token"):
    scope = {
        "type": "http",
        "method": method,
        "path": path,
...
def test_require_auth_blocks_write_for_read_only_role(monkeypatch: pytest.MonkeyPatch):
    ...
    async def run():
        gen = auth_mod.require_auth(_make_request("POST"))
        with pytest.raises(HTTPException) as excinfo:
            await gen.__anext__()
...
def test_require_auth_allows_get_for_read_only_role(monkeypatch: pytest.MonkeyPatch):
    ...
    async def run():
        gen = auth_mod.require_auth(_make_request("GET"))
