...
│   ├── controlling_reports.html  ← Controlling: riportok — valós szűrők + 4 riporttípus (projekt heti+kumulált; személy/ügyfél/tevékenység típus soronkénti listával + Összesítés kártyával), fejléc-sáv (Projekt/Ügyfél/Személy/Tevékenység típus/Időszak), stílusozott PDF export, DataTables Buttons export (Excel/PDF/Print, Összesítéssel)
...
| Riportok | `/ui/controlling/reports` | Valós adat — 4 riporttípus a `timesheet_entry` felett: Projekt riport (heti + kumulált, tevékenység típusonkénti bontással, futó összeggel), Személy riport, Ügyfél riport, Tevékenység típus riport. Az utóbbi három soronkénti listát mutat (Dátum, Nap, Hét, Résztvevők, Óra, plusz Személy/Tevékenység típus oszlop attól függően, hogy az adott szűrő be van-e állítva — ha igen, az oszlop helyett a fejlécben jelenik meg), a tábla alatt egy külön "Összesítés" kártyával (a `report.rows` csoportosított totáljai, tevékenység típusonkénti pivot oszlopokkal). Fejléc-sáv minden riportnál: Projekt / Ügyfél / Személy / Tevékenység típus / Időszak — a beállított szűrő értékét vagy "Összes ...", ill. a ténylegesen felbontott dátumtartományt mutatja. Szűrők: dátumtartomány (projekt kezdete óta / aktuális hónap / aktuális hét / egyéni), ügyfél, projekt, személy, tevékenység típus. Projekt riportnál projekt hiányában automatikusan az első projekt kerül kiválasztásra. Export (Excel/PDF/Nyomtatás) kliens-oldali DataTables Buttons-szal — a ténylegesen szűrt/rendezett táblát exportálja, a fejléc-sávot (`messageTop`) és a nem-projekt riportoknál az Összesítés szekciót is (`exportOptions.customizeData`, a DOM-ból olvasva). A PDF export egyedi stílust kap (`customize` callback: márkaszín fejléc/cím, zebra-csíkozás, kiemelt összesítő sorok, lábléc oldalszámmal); a stílusozás try/catch-csel védett, hiba esetén sima (stílus nélküli) PDF-et exportál, nem hiúsítja meg a letöltést. A mockupban szereplő külön "Heti export" (Excel heti blokk szerkezet) riporttípus továbbra sincs megvalósítva |
