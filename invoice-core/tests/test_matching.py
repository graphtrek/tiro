...
class TestAmountCandidates:
    ...
    def test_huf_grouped_and_plain(self):
        t = _txn(transaction_id="x", amount=3400, description="3 400,00 HUF értékű kártyahasználat")
    ...
    def test_fee_adjusted_net_amount(self):
        ...
        t = _txn(transaction_id="x", amount=94624, fees=424, description="Küldött utalás")
        cands = _amount_candidates(t)
...
def test_amount_disambiguates_online_vs_scaleway(mdb):
    ...
    t_online = _txn(
        ...
        amount=5434.98,
        counterparty_name="Online PARIS",
...
def test_two_identical_charges_get_distinct_files(mdb):
    f1 = InvoiceFile(
        filename="2026-05-29_0012_KC0126-01204.pdf", words="3.400,00 3400 25956537-2-13 forras.net"
    )
    f2 = InvoiceFile(
        filename="2026-05-29_0013_KC0126-01205.pdf", words="3.400,00 3400 23118342-2-43 forras.net"
    )
    mdb.add_all([f1, f2])
    mdb.flush()
    t1 = _txn(
        transaction_id="CARD-A",
        amount=3400,
        counterparty_name="Simplep*Fizetes Budapest",
        transaction_date=datetime(2026, 5, 29),
        description=(
            "3 400,00 HUF értékű kártyahasználat ennél a kereskedőnél: Simplep*Fizetes (Budapest)"
        ),
    )
    t2 = _txn(
        transaction_id="CARD-B",
        amount=3400,
        counterparty_name="Simplep*Fizetes Budapest",
        transaction_date=datetime(2026, 5, 29),
        description=(
...
def test_no_confident_match_stays_null(mdb):
    ...
    t = _txn(
        transaction_id="CARD-3",
        amount=5772.36,
        counterparty_name="Google Workspace_graphtre Dublin",
        transaction_date=datetime(2026, 6, 1),
        description=(
            "16,20 EUR értékű kártyahasználat ennél a kereskedőnél: "
            "Google Workspace_graphtre (Dublin)"
        ),
    )
    mdb.add(t)
    mdb.flush()
...
def test_reference_present_but_absent_stays_null(mdb):
    ...
    other = InvoiceFile(
        filename="2026-06-04_0022_GRPHT-2026-13.pdf", words="127000 uniomedia 2026-13 budapest"
    )
    mdb.add(other)
    mdb.flush()
    t = _txn(
        transaction_id="TRANSFER-8",
        amount=127000,
        direction="CREDIT",
        currency="HUF",
        payment_reference="GRPHT-2026-11",
        counterparty_name="UNIOMEDIA KOMMUNIKÁCIÓS ÜGYNÖKSÉG",
...
def test_file_assigned_then_backlinked_in_same_call(mdb):
    """Phase 2 assigns the file; Phase 3 links the invoice in the same sync_match call."""
...
def test_locked_invoice_not_relinked(mdb):
    ...
    other_file = InvoiceFile(
        filename="2026-06-01_0001_GRPHT-2026-1.pdf", words="GRPHT-2026-1 graphtrek"
    )
