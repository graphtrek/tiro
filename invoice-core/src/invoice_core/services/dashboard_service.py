...
def get_kpis(db: Session) -> DashboardKpis:
    ...
    recent_wise_count = (
        ...
        .filter(WiseTransaction.transaction_date >= cutoff)
        .scalar()
        or 0
    )
    ...
    return DashboardKpis(
        ...
        recent_wise_count=recent_wise_count,
...
def get_recent_transactions(db: Session, limit: int = 5) -> list[RecentTransactionRow]:
    rows = (
        db.query(WiseTransaction, Invoice.invoice_number)
...
def _to_sync_log_row(log: SyncLog) -> SyncLogRow:
    ...
    return SyncLogRow(
        ...
        wise_count=log.wise_count or 0,
