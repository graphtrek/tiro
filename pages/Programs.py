...
with st.expander("➕ Generate a new program", expanded=True):
    with st.form("generate_form"):
        col1, col2 = st.columns(2)
        with col1:
            prog_name = st.text_input("Program name", placeholder="e.g. currency-converter")
            mode = st.selectbox("Execution mode", ["service", "on_demand"])
        with col2:
